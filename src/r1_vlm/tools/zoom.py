...
def zoom(
    ...
    **kwargs,
    ...
    Args:
        ...
        bbox: tuple[int, int, int, int], the bounding box to zoom in on. The bounding box is in the format of [x_min, y_min, x_max, y_max],
    ...
    if not isinstance(bbox, tuple) or len(bbox) != 4:
        raise ValueError("Invalid bbox: must be a tuple of 4 integers")
    ...
    if x_min < 0 or x_min >= width or x_max < 0 or x_max > width:
        raise ValueError(
            f"Invalid bbox: x_min and x_max must be within the image width. x_min: {x_min}, x_max: {x_max}, width: {width}"
        )

    if y_min < 0 or y_min >= height or y_max < 0 or y_max > height:
        raise ValueError(
            f"Invalid bbox: y_min and y_max must be within the image height. y_min: {y_min}, y_max: {y_max}, height: {height}"
        )

    if x_min >= x_max or y_min >= y_max:
        raise ValueError(
            f"Invalid bbox: x_min must be less than x_max and y_min must be less than y_max. x_min: {x_min}, x_max: {x_max}, y_min: {y_min}, y_max: {y_max}"
        )

    # validate the magnification
    if not isinstance(magnification, (float, int)) or float(magnification) < 1.0:
        raise ValueError(
            "Invalid magnification: must be a float greater than or equal to 1.0"
        )

    # crop the image to the bounding box
    cropped_image = image.crop(bbox)

    # resize the image according to magnification if needed
    if magnification > 1.0:
        new_size = tuple(int(dim * magnification) for dim in cropped_image.size)

        MAX_SIZE = (300, 300)
        # Scale down if either dimension exceeds maximum allowed
        if new_size[0] > MAX_SIZE[0] or new_size[1] > MAX_SIZE[1]:
            scale = min(MAX_SIZE[0] / new_size[0], MAX_SIZE[1] / new_size[1])
            new_size = tuple(int(dim * scale) for dim in new_size)
...
def test_invalid_bbox_type(sample_image):
    with pytest.raises(ValueError, match="must be a tuple"):
        zoom("test_image", [0, 0, 50, 50], 1.0, images=sample_image)


def test_invalid_bbox_length(sample_image):
    with pytest.raises(ValueError, match="must be a tuple of 4 integers"):
...
def test_basic_zoom_no_magnification(sample_image):
    ...
    assert result.size == (20, 20)  # Should match bbox dimensions
