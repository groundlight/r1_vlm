...
from r1_vlm.datasets.aok_vqa.aok_vqa_da_tool_use_r1 import (
    create_r1_aok_vqa_da_tool_use_dataset,
)
...
class AOKVQA_DA_ToolEnv(ToolVisionEnv):
    def __init__(
        self,
    ...
    def get_dataset(self) -> tuple[Dataset, Dataset, Dataset]:
        dataset = create_r1_aok_vqa_da_tool_use_dataset()
    ...
    def get_rubric(self) -> list[RewardFunc]:
        def format_reward_func(prompts, completions, completions_messages, **kwargs):
            ...
            preprocessed_data = AOKVQA_DA_ToolEnv.preprocess_for_reward(
                prompts=prompts,
        ...
        def check_correctness(conversation, direct_answers) -> float:
            ...
            if hasattr(parsed, "answer"):
                ...
                for direct_answer in direct_answers:
                    dist = Levenshtein.distance(direct_answer, answer)
                    normalized_dist = dist / max(len(direct_answer), len(answer))
                    score = 1 - normalized_dist
                    scores.append(score)

                top_3_scores = sorted(scores, reverse=True)[:3]
                total_score = sum(top_3_scores)
                score = min(total_score / 3.0, 1.0)
                return score
        ...
        def correct_answer_reward_func(
            ...
            # scoring process is a soft version of the evaluation function:
            # 1. Compute (1 - normalized edit distance) between the model's answer and each of the direct answers. This is the score for each direct answer.
            # 2. Take the top 3 scores.
            # 3. Sum the top 3 scores.
            # 4. Divide the sum by 3.0.
            # 5. Take min of this value and 1.0.

            correctness_rewards: list[float] = [
                check_correctness(conversation=conv, direct_answers=direct_answers)
...
if __name__ == "__main__":
    env = AOKVQA_DA_ToolEnv(processing_class=None)
