...
def find_examples_for_training_less_than_0_5_VQA_score(
    eval_results_file_path: str = "/millcreek/home/sunil/r1_vlm/src/r1_vlm/environments/tool_use_text_vqa_env/eval_on_train_results_8shot.jsonl",
):
    """
    Returns a dataset of question_ids where the average VQA score on the base model 8 shot is less than or equal to 0.5
    """
    ...
    # compute the average vqa score for each question_id
    for question_id in results_by_question_id:
        results = results_by_question_id[question_id]["results"]
        average_vqa_score = sum([result["vqa_score"] for result in results]) / len(
            results
        )
        results_by_question_id[question_id]["average_vqa_score"] = average_vqa_score

    # choose the examples with average vqa score <= 0.5
    question_ids = [
        question_id
        for question_id in results_by_question_id
        if results_by_question_id[question_id]["average_vqa_score"] <= 0.5
    ]
