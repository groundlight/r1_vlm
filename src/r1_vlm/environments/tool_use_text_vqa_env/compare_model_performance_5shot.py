...
if __name__ == "__main__":
    # zoom model
    trained_model_eval = "/millcreek/home/sunil/r1_vlm/src/r1_vlm/environments/tool_use_text_vqa_env/eval_zoom_step800_validation_results_no_structured_output_5shot.jsonl"
    ...
    for question_id in tqdm(
        ...
        # get the 5 shot result for the trained model
        trained_results = [
            result
        ...
        assert len(trained_results) == 5, (
            f"Trained model should have 5 results for question id: {question_id}"
        )

        trained_scores = [result["vqa_score"] for result in trained_results]
        trained_score = sum(trained_scores) / len(trained_scores)

        # collate them into a single dictionary
        result = {
            "question_id": question_id,
            "base_model_result": base_result["vqa_score"],
            "trained_model_result": trained_score,
        }
