...
from r1_vlm.datasets.aok_vqa.aok_vqa_mc_tool_use_datamixing_r1 import (
...
class AOKAndTextVQAToolEnv(ToolVisionEnv):
    def __init__(
        self,
    ...
    def get_dataset(
        ...
        """
        Max examples per split is on a per dataset basis.
        Max size is used to resize images for textVQA and does not apply to AOK dataset.
        skip_index is applied to the "merged" training dataset after the datasets are combined.
        """

        def create_text_vqa_dataset(
        ...
        text_vqa_datasets = create_text_vqa_dataset(
            splits_to_process=splits,
            max_examples_per_split=max_examples_per_split,
            max_size=max_size,
        )

        aok_vqa_datasets = create_aok_vqa_dataset(
            splits_to_process=splits,
            max_examples_per_split=max_examples_per_split,
        )

        final_datasets = {}
        # now we need to merge each split
        for split in splits:
            text_vqa_dataset = text_vqa_datasets[split]
            aok_vqa_dataset = aok_vqa_datasets[split]

            # for train, we will interleave the datasets
            if split == "train":
        ...
        if skip_index is not None:
            final_datasets["train"] = final_datasets["train"].select(
                range(skip_index, len(final_datasets["train"]))
            )
    ...
    def get_rubric(self) -> list[RewardFunc]:
        def format_reward_func(prompts, completions, completions_messages, **kwargs):
            ...
            preprocessed_data = AOKAndTextVQAToolEnv.preprocess_for_reward(
                prompts=prompts,
        ...
        def correct_answer_reward_func(
            ...
            """
            Reward function that checks if the model's answer is correct.
            """

            # check which dataset the example comes from
            ...
            if dataset_name == "textvqa":
                ...
                soft_rewards = soft_correctness_reward_func(
                    prompts, completions, completions_messages, **kwargs
                )
                rewards = [
                    (hard_reward + soft_reward) / 2
                    for hard_reward, soft_reward in zip(hard_rewards, soft_rewards)
                ]
                return rewards

            # AOKs reward is just hard correctness
            elif dataset_name == "aok_vqa":
                return correct_answer_reward_func_aokvqa(
                    prompts, completions, completions_messages, **kwargs
                )
            else:
                raise ValueError(
        ...
        def check_correctness_textvqa(conversation, correct_answers) -> bool:
            """
            Computes vqa score for textvqa
            """
        ...
        def correct_answer_reward_func_textvqa(
            prompts, completions, completions_messages, **kwargs
        ) -> list[float]:
            """
            Provides a reward if the model's answer is correct (textvqa only)
            """
            merged_completion_conversations = MultistepVisionEnv.preprocess_messages(
                prompts_messages=prompts, completions_messages=completions_messages
            )
            ...
            for conv in merged_completion_conversations:
                ...
                correctness_rewards.append(
                    check_correctness_textvqa(conv, correct_answers)
                )
        ...
        def correct_answer_reward_func_aokvqa(
            prompts, completions, completions_messages, **kwargs
        ) -> list[float]:
            """
            Computes multiple choice correctness score for aokvqa
            """
            merged_completion_conversations = MultistepVisionEnv.preprocess_messages(
                prompts_messages=prompts, completions_messages=completions_messages
            )

            correct_answers_list = kwargs["multiple_choice_answer_letter"]
            if len(set(correct_answers_list)) > 1:
                raise ValueError(
                    f"All answers in the multiple choice answer list should be the same, but got {correct_answers_list}"
                )

            # parse the model answers
            model_answers = []
            for conversation in merged_completion_conversations:
                ...
                parsed = self.parser.parse(text)
                if hasattr(parsed, "answer") and parsed.answer is not None:
                    model_answers.append(parsed.answer)
                else:
                    model_answers.append(None)

            # check for correctness
            rewards = []
            for model_answer, correct_answer in zip(
                model_answers, correct_answers_list
            ):
    ...
    def log_metrics(
        ...
        for advantage, messages in zip(advantages, completion_messages):
            ...
            with open(
                "/millcreek/home/sunil/r1_vlm/src/r1_vlm/environments/tool_use_aok_text_vqa_env/tool_use_metrics.jsonl",
                "a",
            ) as f:
                data_point = {
                    "advantage": advantage,
