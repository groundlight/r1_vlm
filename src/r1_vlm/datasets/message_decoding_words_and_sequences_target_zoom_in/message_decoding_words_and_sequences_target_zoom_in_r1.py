...
def generate_r1_messages(example):
    ...
    ending = (
        "You are very bad at determining this directly from the image, because some targets of the key mappings in the decoder are too small to see."
        "Given the source character of the mapping you need to decode, you should be able to determine where the bounding box where the tool should be applied."
        "Instead, please use the tools available to you to solve this problem. Theoretically, you should be able to sove the problem by using the tool once. However, if your first attempt failed, you can try using the tool again."
    )
